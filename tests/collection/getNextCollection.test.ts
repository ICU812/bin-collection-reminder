...
describe("getNextCollections", () => {
  ...
  it("returns the earliest date and correct bins", () => {
    const { nextCollectionDate, nextCollections } =
      getNextCollections(mockCollections);
  ...
  it("throws if there are no collections", () => {
    expect(() => getNextCollections([])).toThrow(
  ...
  it("throws if jsDate is missing", () => {
    const broken = [
      { ...mockCollections[0], jsDate: undefined },
    ] as unknown as BinCollection[];
